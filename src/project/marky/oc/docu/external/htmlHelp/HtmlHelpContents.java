...
public class HtmlHelpContents extends StdHtmlFile
{
	...
	private void writeNamespaceEntries(final File outputFolder, final StdNamespace namespace)
	{
		...
		if (writeHeader) writeLine(_UL, level);
	}


	private boolean isGlobalNamespace(final StdNamespace namespace)
	{
		return _parser.getNameSpaces().getNamespaceGlobal().getIdentifier().equals(namespace.getIdentifier());
	}


	private boolean isDocuNamespace(final StdNamespace namespace)
	{
		return _parser.getNameSpaces().getNamespaceDocu().getIdentifier().equals(namespace.getIdentifier());
	}


	private void writePageEntries(final File outputFolder, final StdNamespace namespace, final DocuPage page)
	{
		final int level = 3;
	...
	public void createContent(final File outputFolder, final File defaultFile, final String title)
	{
		writeHeader();
